...
## How it Works
LED and LCD components are identified based on the associated pins on the Arduino UNO R3 and the times for each light are pre-set. The program loops and keeps track of the amount of time left for each state. During this time, messages on the Liquid Crystal Display appear identifying which state the program is currently in, and music is played during the red-light state. After a full traffic cycle occurs the loop resets all values all the program to continue to the following cycle. 
